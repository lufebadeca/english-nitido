...
import { spanishLikeWords, techWords, commonVerbs } from '../data/vocabularyData';
import { audioManager } from '../utils/audioUtils';

const PracticePage: React.FC = () => {
  const [selectedCategory, setSelectedCategory] = useState<'all' | 'spanish-like' | 'tech' | 'verbs'>('all');
  const [practiceMode, setPracticeMode] = useState<'listen' | 'speak' | 'quick'>('listen');
  ...
  const getWordsForCategory = () => {
    switch (selectedCategory) {
      case 'spanish-like': return spanishLikeWords;
      case 'tech': return techWords;
      case 'verbs': return commonVerbs;
  ...
  const categories = [
    ...
    { id: 'spanish-like', name: 'Palabras Similares', count: spanishLikeWords.length },
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
            ...
            >
              <type.icon 
                ...
                className={`mx-auto mb-2 ${
                  practiceMode === type.id ? `text-${type.color}-500` : 'text-gray-400'
                }`} 
              />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {categories.map((category) => (
            ...
            >
              ...
              <div className="text-sm text-gray-600">{category.count} palabras</div>
            </button>
          ))}
        </div>
      </motion.div>
      ...
      >
        <div className="text-center space-y-6">
          <div className="space-y-2">
            ...
            <h3 className="text-4xl font-bold text-gray-800">{currentWord.en}</h3>
            <div className="text-blue-600 font-mono text-xl">{currentWord.ipa}</div>
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <button
              onClick={() => audioManager.speakSentence(currentWord.examples[0])}
          ...
          {currentWord.mnemonic && (
            <div className="bg-purple-50 rounded-lg p-4">
              <h4 className="font-semibold text-purple-800 mb-2">💡 Truco de memoria:</h4>
      ...
      >
        <h3 className="text-lg font-bold text-blue-800 mb-3">💡 Consejos para la Práctica</h3>
        <ul className="space-y-2 text-blue-700">
          <li>• Escucha cada palabra varias veces antes de intentar pronunciarla</li>
