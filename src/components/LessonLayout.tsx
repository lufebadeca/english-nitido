      ...
      <div className="space-y-6">
        ...
        >
          ...
          <p className="text-lg text-gray-600">
            Aprende las reglas de la lección
          </p>
        </motion.div>
            ...
            {rule.explanation.length > 1 && (
              <ul className="space-y-1 bg-yellow-50 rounded-lg p-4">
                {rule.explanation.map((explanationRow, idxpz) => {
                  if (idxpz !== 0) {
                    return (
                      <li key={idxpz} className="text-gray-700">
                        • {explanationRow}
                      </li>
                    );
                  }
      ...
      return (
        <div className="text-center space-y-4">
          <p className="text-gray-600">
            Esta lección no incluye vocabulario específico.
          </p>
          <button
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            <button
              ...
              className="text-xs md:text-sm px-4 py-2 bg-gray-100 text-gray-600 rounded-lg hover:bg-gray-200 transition-colors duration-200"
            >
              {showTranslations
        ...
        <div className="w-full bg-gray-200 rounded-full h-2">
          <motion.div
            ...
            animate={{
              width: `${
                ((currentVocabIndex + 1) / lesson.vocabulary.length) * 100
              }%`,
            }}
        ...
        <div className="flex justify-between">
          <button
            onClick={() =>
              setCurrentVocabIndex(Math.max(0, currentVocabIndex - 1))
            }
        ...
        >
          ...
          >
            ...
            <p className="text-lg text-gray-600 mb-4">
              Obtuviste {quizScore} de {maxScore} puntos (
              {Math.round(percentage)}%)
            </p>

            {percentage >= 70 ? (
              <p className="text-green-700">
                ¡Has completado la lección exitosamente! Continúa con la
                siguiente lección.
              </p>
            ) : (
              <p className="text-yellow-700">
                Necesitas al menos 70% para completar la lección. ¡Inténtalo de
                nuevo!
              </p>
            )}
          </div>
        ...
        {["info", "vocab", "quiz"].map((stage, index) => (
          <div key={stage} className="flex items-center">
            ...
            <div className="flex flex-col md:flex-row items-center ">
              ...
              {index < 2 && (
                <div
                  className={`w-14 md:w-10 h-0.5 mx-2 md:mx-4 ${
                    index < ["info", "vocab", "quiz"].indexOf(currentStage)
                      ? "bg-green-500"
                      : "bg-gray-200"
                  }`}
